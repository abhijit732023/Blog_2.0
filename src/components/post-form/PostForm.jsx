...
export default function PostForm({ post }) {
    ...
    return (
        <form onSubmit={handleSubmit(submit)} className="flex justify-center h-full px-4">
            <motion.div 
                ...
                className="w-full max-w-4xl h-full flex flex-col md:flex-row justify-around bg-white pb-4 pt-4 rounded-md shadow-lg"
            >
                <div className="w-full md:w-2/5 bg-gray-200 p-3 rounded-md">
                    ...
                    {post && <img src={appwriteService.getFilePreview(post.featuredimage)} alt={post.title} className="rounded-lg mb-4 w-full" />}
                    <Select options={["active", "inactive"]} label="Status" className="mb-4 shadow-md" {...register("status", { required: true })} />
                    <Button type="submit" bgColor={post ? "bg-green-500" : undefined} className="w-full mt-4 shadow-md">
                        {post ? "Update" : "Submit"}
                    </Button>
                </div>
                <div className="w-full md:w-3/5 bg-gray-200 p-2 rounded-md mt-4 md:mt-0">
